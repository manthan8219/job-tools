...
export class DynamiteJobsJobScraperAdapter extends BaseJobScraperAdapter {
  readonly name = "DynamiteJobsJobScraperAdapter";
  readonly source = "dynamitejobs";
  ...
  constructor(options?: { feedUrl?: string; timeoutMs?: number }) {
    super();
    this.feedUrl = options?.feedUrl ?? "https://www.dynamitejobs.com/feed/";
  ...
  protected async executeScrape(query: ScrapeQuery): Promise<ScrapeResult> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`DynamiteJobs RSS feed responded with HTTP ${response.status}`);
      }
