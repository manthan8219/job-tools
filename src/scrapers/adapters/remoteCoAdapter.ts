...
export class RemoteCoJobScraperAdapter extends BaseJobScraperAdapter {
  readonly name = "RemoteCoJobScraperAdapter";
  readonly source = "remoteco";
  ...
  constructor(options?: { feedUrl?: string; timeoutMs?: number }) {
    super();
    this.feedUrl = options?.feedUrl ?? "https://remote.co/remote-jobs/feed/";
  ...
  protected async executeScrape(query: ScrapeQuery): Promise<ScrapeResult> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Remote.co RSS feed responded with HTTP ${response.status}`);
      }
      ...
      for (const item of items) {
        ...
        // Remote.co format: "Job Title at Company"
        let company = "Remote.co";
        let title = rawTitle;
