...
export class EuroRemoteJobsJobScraperAdapter extends BaseJobScraperAdapter {
  readonly name = "EuroRemoteJobsJobScraperAdapter";
  readonly source = "euroremotejobs";
  ...
  constructor(options?: { feedUrl?: string; timeoutMs?: number }) {
    super();
    this.feedUrl = options?.feedUrl ?? "https://euroremotejobs.com/feed/";
  ...
  protected async executeScrape(query: ScrapeQuery): Promise<ScrapeResult> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`EuroRemoteJobs RSS feed responded with HTTP ${response.status}`);
      }
